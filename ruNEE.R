...
# Get basic reporting template, fill with any existing info (excludes episodes)
...
bsi_df <- bsi_df %>% dplyr::mutate(BSIisolates = paste0(AdmissionRecordId,OnsetDate,MicroorganismCode)) %>%
  dplyr::distinct()
...
calc_df <- orig_df %>%
  dplyr::select(EpisodeId, EpisodeOrigin) %>%
  dplyr::filter(!is.na(EpisodeId)) %>%
  dplyr::distinct()
...
aggregateResults <- orig_df %>%
  dplyr::select(HospitalId, EpisodeClass, EpisodeId) %>%
  dplyr::distinct() %>%
  dplyr::group_by(HospitalId, EpisodeClass) %>%
  dplyr::mutate(countEps = dplyr::n()) %>%
  dplyr::select(HospitalId, EpisodeClass, countEps) %>%
  dplyr::distinct() %>%
  tidyr::pivot_wider(names_from = EpisodeClass,
              values_from = countEps,
              id_cols = HospitalId) %>%
  dplyr::mutate(NumberOfCABSIs = CA
...
ehrbsi <- ehrbsi %>%
  dplyr::select(-NumberOfTotalBSIs,-NumberOfHOHABSIs,-NumberOfImportedHABSIs) %>%
  dplyr::left_join(aggregateResults, by = c("RecordId"="HospitalId")) %>%

...
# Load report data from RDS file (avoids YAML serialization issues with NA values).
# Falls back to params$report_data for backwards compatibility with direct parameter passing.
if (!is.null(params$report_data_path) && file.exists(params$report_data_path)) {
  report_data_loaded <- readRDS(params$report_data_path)
} else if (!is.null(params$report_data)) {
  report_data_loaded <- params$report_data
} else {
  report_data_loaded <- list()
}
# Alias so the rest of the document can keep using params$report_data syntax
params_report_data <- report_data_loaded
...
# Get data from loaded report data
raw_stats <- params_report_data$raw_stats
processed_stats <- params_report_data$processed_stats
...
```{r data-cleaning-pie, fig.width=7, fig.height=5.5}
...
if (!is.null(params_report_data$patient)) {
  has_demographics <- all(c("Age", "Sex") %in% names(params_report_data$patient))
}
```

```{r demographics-content, eval=has_demographics}
patient_data <- params_report_data$patient

# Display demographics summary
cat(generate_demographics_summary(patient_data, for_markdown = TRUE))
```

```{r demographics-plots, eval=has_demographics, fig.show='hold', out.width='48%', fig.width=5, fig.height=4.5}
# Gender distribution
...
has_episodes <- !is.null(params_report_data$episodes)
if (has_episodes) {
...
```{r episodes-composition-pies, eval=has_episodes, fig.show='hold', out.width='32%', fig.width=3.5, fig.height=4}
...
```{r monomicrobial-pathogens, eval=has_episodes, fig.width=10, fig.height=6}
...
```{r monomicrobial-unavailable, eval=has_episodes && is.null(params_report_data$episode_summary)}
cat("**Note:** Detailed pathogen analysis requires episode summary data which is not currently available.\n")
```

### Polymicrobial Episodes

```{r polymicrobial-pathogens, eval=has_episodes, fig.show='hold', out.width='48%', fig.width=5, fig.height=5.5}
episode_summary <- as_df_safe(rd$episode_summary)
has_summary <- !is.null(episode_summary) && tryCatch(NROW(episode_summary) > 0, error = function(e) FALSE)
if (has_summary) {
...
```{r polymicrobial-unavailable, eval=has_episodes && is.null(params_report_data$episode_summary)}
cat("**Note:** Detailed pathogen analysis requires episode summary data which is not currently available.\n")
```
...
```{r infection-type-pies, eval=has_episodes, fig.show='hold', out.width='32%', fig.width=3.5, fig.height=4}
...
```{r commensal-summary, eval=has_episodes}
cat(generate_commensal_summary(episodes_data, for_markdown = TRUE))
```

```{r commensal-pies, eval=has_episodes, fig.show='hold', out.width='32%', fig.width=3.5, fig.height=4}
print(plot_commensal_pie(episodes_data, filter_class = NULL))
print(plot_commensal_pie(episodes_data, filter_class = "HA"))
print(plot_commensal_pie(episodes_data, filter_class = "CA"))
```
...
```{r specialty-pies, eval=has_context, fig.show='hold', out.width='48%', fig.width=5, fig.height=5}
...
```{r pathogen-specialty, eval=has_context, fig.width=10, fig.height=6}
...
```{r timeseries-check, include=FALSE}
has_timeseries <- FALSE
if (!is.null(rd$episode_summary)) {
  es <- as_df_safe(rd$episode_summary)
  if (!is.null(es) && nrow(es) > 0 && "EpisodeStartDate" %in% names(es)) {
    has_timeseries <- TRUE
  }
}
```

```{r timeseries-content, eval=has_timeseries, include=FALSE}
episode_summary_ts <- as_df_safe(rd$episode_summary)
```

## Episodes Over Time

```{r timeseries-plot, eval=has_timeseries, fig.width=10, fig.height=5.5}
print(plot_timeseries(episode_summary_ts, aggregation = "month", show_trend = TRUE))
```

## Seasonal Pattern

```{r seasonality-plot, eval=has_timeseries, fig.width=10, fig.height=5}
print(plot_seasonality(episode_summary_ts))
```

## Summary Statistics

```{r timeseries-summary, eval=has_timeseries}
cat(generate_timeseries_summary(episode_summary_ts, aggregation = "month", for_markdown = TRUE))
```

```{r timeseries-unavailable, eval=!has_timeseries}
cat("Time series analysis requires episode summary data with EpisodeStartDate.")
```
...
rd_h <- if (exists("rd")) rd else params_report_data
...
```{r hospital-ward-plots, eval=has_hospital_analysis, fig.width=10, fig.height=5.5}
